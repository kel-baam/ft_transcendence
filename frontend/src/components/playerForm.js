...
export function playerForm() {
    ...
    formClass.onsubmit = (event) => {
        ...
        // Correctly select the form element using its class
        ...
        if (formElement1) {
            ...
            // Log form data entries (debugging purposes)
            for (const [key, value] of formData1.entries()) {
                console.log(`${key}: ${value}`);
            }

            // Remove the form element from the DOM (if `playerForm` is defined)
            if (playerForm) {
                document.body.removeChild(playerForm);
            }

            return formData1; // Note: This return will not propagate outside the event handler
        } else {
    ...
    avatar.src = './images/niboukha.png';
