...
export const LocalHierarchy = defineComponent({
    ...
    async matchmake_players()
    {
        if (!this.state.socket || this.state.socket.readyState !== WebSocket.OPEN) {
            ...
            this.state.socket   = new WebSocket('ws://localhost:8000/ws/matchmaking/');
                ...
                h('div', { class: 'hierarchy-global-content' }, [
                        ...
                        h('div', { class: 'round1' }, 
                            (this.state.matcheRounds || []).map((match, i) =>
                                h('div', { class: `match${i + 1}` }, [
                                    h('div', { class: 'player1' }, [
                                        h('img', { 
                                            src: match.avatar1 ? `http://localhost:8000${match.avatar1}` : './default-avatar.png' 
                                        }),
                                        h('h2', {}, [match.player1])
                                    ]),
                                    h('div', { class: 'vs' }, [
                                    ...
                                    h('div', { class: 'player2' }, [
                                        h('img', { 
                                            src: match.avatar2 ? `http://localhost:8000${match.avatar2}` : './default-avatar.png' 
                                        }),
                                        h('h2', {}, [match.player2])
                                    ])
                                ])
                    ...
                    [
                        h('div', { class: 'user-profile' }, [
                            h('img', { src: `http://localhost:8000${this.state.currentMatch.avatar1}` }),
