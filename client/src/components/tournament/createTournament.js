...
export const CreateTournament = defineComponent({
    ...
    async friends_list(event) {
        ...
        try {
            ...
            const response      = await customFetch(`https://${window.env.IP}:3000/api/tournament/online/tournaments/friends-list?search=${encodeURIComponent(searchQuery)}&playersLength=${playersLength}`,
            {
                method      : 'GET',
                credentials : 'include',
            });
